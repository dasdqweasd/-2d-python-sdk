...
def check_sys_and_update_dll():
    ...
    # 1. 动态获取当前 .py 文件所在的绝对目录
    current_dir = os.path.dirname(os.path.abspath(__file__))

    # 2. 拼接出同级目录下的 bin 文件夹路径
    custom_bin_dir = os.path.join(current_dir, "bin")

    max_size = sys.maxsize
    bit_info = "64" if max_size > 2 ** 32 else "32"
    currentsystem = platform.system()

    if currentsystem == 'Windows':
        # 3. 魔改核心：将 bin 目录临时加入当前 Python 进程的 PATH 最前端！
        # 这样 MvCameraControl.dll 在内部加载 GenICam 依赖时就不会迷路了
        os.environ['PATH'] = custom_bin_dir + os.pathsep + os.environ.get('PATH', '')

        # 4. 精确指定主 DLL 的绝对路径
        MvCamCtrldllPath = os.path.join(custom_bin_dir, "MvCameraControl.dll")

        if not os.path.exists(MvCamCtrldllPath):
            raise FileNotFoundError(f"【致命错误】找不到海康底层驱动！请确保存在此路径: {MvCamCtrldllPath}")
